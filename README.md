...
The B2B marketing website for **StretchMetal** — the metal-fabrication business unit of the Belgian Stretchgroup, operating from Częstochowa, Poland. The site's one job is to get a prospect to send a technical drawing through the RFQ form (`/wycena` · `/en/quote`). Built with Next.js 15 (App Router) + React 19 + TypeScript (strict) + Tailwind CSS v4, bilingual — Polish at the root (default), English mirror under `/en` — and deployed on Vercel with **zero required environment variables**.
...
```
/app                       Routes — Polish tree at the root, English mirror under /en
  ...
  sitemap.ts               Emits both locale trees with hreflang alternates
...
Every page must include metadata (unique title ≤ 60 chars, description ≤ 155 chars, canonical + hreflang) and breadcrumb JSON-LD. Route paths come from `lib/i18n-routes.ts` — never hardcode URLs. Pattern (Polish tree; the `/en` twin is identical with `.en` paths and English copy):
...
Additional schema rules: service pages use `servicePaths(entry)` for the canonical and add `buildService` + `buildFaqPage`; home and contact add `buildLocalBusiness`; thank-you pages set `robots: { index: false, follow: false }` and stay out of the sitemap.

## Content & the `[CONFIRM]` policy

Components are copy-free — every string a visitor reads lives in typed TS files under `/content` (Polish) and `/content/en` (English), both satisfying `content/types.ts` with identical export names and shapes. Business facts the owner has not yet verified (legal name, address, phone, domain, capacity, machine specs, spec-table values, …) carry a greppable `// [CONFIRM]` comment on the line where they appear. Find every unverified value in one pass:
...
3. After the domain purchase, set `NEXT_PUBLIC_SITE_URL` and redeploy — canonicals, hreflang, sitemap and JSON-LD all follow it.
