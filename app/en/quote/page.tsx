/**
 * RFQ page — /en/quote
 * File path: /app/en/quote/page.tsx
 *
 * English mirror of /app/wycena/page.tsx: identical section rhythm, copy
 * from /content/en/rfq.ts, routes/canonicals on the EN side. Written for
 * a German/Benelux purchasing manager evaluating a Polish supplier — the
 * trust rail (confidentiality, NDA, engineer contact) carries the page's
 * argument as much as the form does.
 *
 * Layout: dark hero → surface two-column shell (<RfqForm> in a white
 * hairline card, sticky trust rail with red .tick squares) → mini 5-step
 * process strip → FAQ (details/summary accordion + FAQPage JSON-LD) →
 * dark contact strip (phone as TrackedCTA `phone_click` {location:"rfq"})
 * → Footer.
 *
 * Deliberately NO <MobileStickyCta>: it advertises this very page (and
 * self-hides on RFQ routes anyway). The page itself fires no analytics —
 * the form owns `rfq_form_start` / `rfq_submitted`. FRAME holds only the
 * furniture RfqContent has no fields for (eyebrow labels, breadcrumb
 * names). Contact data via siteConfig, never hardcoded.
 */
...
import { rfq } from "@/content/en/rfq";
import { nav, footer } from "@/content/en/ui";

/** Page-frame copy with no RfqContent field — kept minimal on purpose. */
const FRAME = {
  breadcrumbHome: "Home",
  breadcrumbSelf: "Request a quote",
        ...
        <JsonLd
          data={buildBreadcrumbs([
            ...
            },
            {
              ...
              url: `${siteConfig.url}${routes.rfq.en}`,
            },
          ])}
        />
